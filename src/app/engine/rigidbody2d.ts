...
import { Time, Component, GameObject, Vector2, BoxCollider, } from "./qbcreates-js-engine";
...
export class Rigidbody2d extends Component {
    ...
    applyPhysics() {
        if (this.totalForceY.y == 0 && this.normalForceY.y == 2) {
            this.velocityY.y = -.00001;
        }
        let attachedCollider: BoxCollider = this.gameObject.getComponent(BoxCollider);
        ...
        let colliders = Physics2d.BoxCast(new Vector2(this.transform.position.x + attachedCollider.offset.x, this.transform.position.y + (Math.sign(this.velocityY.normalize().y) * ((attachedCollider.scale.y / 2) + .05 + (Math.sign(this.velocityY.normalize().y) * attachedCollider.offset.y)))), new Vector2(attachedCollider.scale.x, 0), Vector2.multiply(Vector2.up, Math.sign(this.velocityY.normalize().y)), Vector2.subtract(newPosY, prevPosY).magnitude);
        ...
        if (colliders.length > 0) {
            ...
            if (Math.sign(this.velocityY.y) == -1) {
                this.transform.position = new Vector2(this.transform.position.x, topY + (attachedCollider.scale.y / 2) - attachedCollider.offset.y);
            } else if (Math.sign(this.velocityY.y) == 1) {
                this.appliedForce = new Vector2(this.appliedForce.x, 0);
                this.transform.position = new Vector2(this.transform.position.x, BottomY - (attachedCollider.scale.y / 2) - attachedCollider.offset.y);
            }
        ...
        let horizontalColliders = Physics2d.BoxCast(new Vector2(this.transform.position.x + (Math.sign(this.velocityX.x) * ((attachedCollider.scale.x / 2) + .1 + (Math.sign(this.velocityX.normalize().x) * attachedCollider.offset.x))), this.transform.position.y + attachedCollider.offset.y), new Vector2(0, attachedCollider.scale.y), Vector2.multiply(Vector2.right, Math.sign(this.velocityX.x)), Vector2.subtract(newPosX, prevPosX).magnitude + .01);
        ...
        if (horizontalColliders.length > 0 && this.totalForceX.x != 0) {
            ...
            if (Math.sign(this.totalForceX.x) == -1) {
                this.transform.position = new Vector2(rightX + (attachedCollider.scale.x / 2) - attachedCollider.offset.x, this.transform.position.y);
            } else if (Math.sign(this.totalForceX.x) == 1) {
                this.transform.position = new Vector2(leftX - (attachedCollider.scale.x / 2) - attachedCollider.offset.x, this.transform.position.y);
